...
class BatteryNode(Node):
    ...
    def __init__(self):
        ...
        self.last_timer_battery_state_changed_ = self.get_current_time_seconds()
    ...
    def check_battery_state(self):
        ...
        if self.battery_state_ == "full":
            if time_now - self.last_timer_battery_state_changed_ > 4.0:
                self.battery_state_ = "empty"
                self.get_logger().info("Battery is empty! Charging battery...")
                self.last_timer_battery_state_changed_ = time_now
                self.call_set_led_server(3, 1)
        else:
            if time_now - self.last_timer_battery_state_changed_ > 6.0:
                self.battery_state_ = "full"
